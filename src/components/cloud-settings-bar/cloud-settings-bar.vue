<template>
  <div v-if="isShowBar" class="cloud-settings-data-box status-bar info" :class="{
    warning: selectedAction === CloudSettingsActions.update,
    success: selectedAction === CloudSettingsActions.equal
  }">
    <div>{{ actionsTip }}</div>
    <el-button type="primary" text :icon="icon.IEpCheck" :loading="saveLoading" :disabled="saveDisabled"
      @click="onConfirm">
      {{ $t('confirm') }}
    </el-button>
  </div>

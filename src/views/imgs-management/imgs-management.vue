<template>
  <div class="page-container management-page-container">
    ...
    <div class="bottom-box border-box" v-loading="loadingImageList"
      :element-loading-text="$t('management_page.loadingTxt1')">
      <image-selector v-if="currentPathImageList.length" :currentDirImageList="currentPathImageList"
        @updateInitImageList="currentPathImageList as any" :key="renderKey"></image-selector>
      <div class="content-list-box border-box" :class="{ 'has-tools': isShowBatchTools }"
        v-contextmenu="{ type: ContextmenuEnum.dirArea }">
        <ul class="dir-card-list list-item border-box" v-if="currentPathDirList.length">
          <li class="dir-card-item border-box" v-for="(dir, idx) in currentPathDirList"
        ...
        <ul class="image-card-list list-item border-box" v-if="currentPathImageList.length">
          <li class="image-card-item border-box" v-for="(image, idx) in currentPathImageList"
            :key="'image-card-' + idx">

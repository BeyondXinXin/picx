...
export const oneClickAutoConfig = async (tokenInput: any) => {
  userConfigInfo.repo = userConfigInfo.owner
  ...
  try {
    ...
    // 判断是否已存在 PicX 图床仓库
    console.log('initRepoInfo')
    let isExistInitRepo: boolean = false
    userConfigInfo.repo = userConfigInfo.owner
    const initRepoInfo = await getRepoInfo(userConfigInfo.token, userConfigInfo.owner, userConfigInfo.owner)
    ...
    console.log('createRepo',userConfigInfo.repo)
    const repoInfo = await createRepo(userConfigInfo.token, userConfigInfo.repo)

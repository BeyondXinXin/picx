...
// const authorize_api = 'http://112.126.73.204:5678/api/github-authorize'
const authorize_api = 'https://api.beyondxin.top/api/github-authorize'
...
export const githubAppAuthorizeCallback = async () => {
  ...
  console.log('githubAppAuthorizeCallback code >> ', code)
  // return
  ...
  if (code) {
    ...
    try {
      ...
      if (!token || (token && isAuthorizeExpire())) {
        ...
        // 在服务端获取 Token
        const res = await axios.get(`${authorize_api}?code=${code}`)
        if (res.data?.token) {
